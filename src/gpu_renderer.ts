import type { AppParams, BackendKind, BackendPreference, GpuInitResult, KRange } from "./types.js";
...
type GlUniforms = {
  ...
  kMin: WebGLUniformLocation | null;
  kMax: WebGLUniformLocation | null;
  normalize: WebGLUniformLocation | null;
...
export class HybridGpuRenderer {
  ...
  private kRange: KRange;

  private gl: WebGL2RenderingContext | null;
  private glProgram: WebGLProgram | null;
  private glTexture: WebGLTexture | null;
  private glUniforms: GlUniforms | null;

  private device: any;
  private wgpuContext: any;
  private wgpuFormat: any;
  private wgpuPipeline: any;
  private wgpuUniformBuffer: any;
  private wgpuBindGroup: any;
  private wgpuTexture: any;
  private wgpuTextureWidth: number;
  private wgpuTextureHeight: number;
  private readonly wgpuUniformData: Float32Array;
  ...
  async initialize(preference: BackendPreference): Promise<GpuInitResult> {
    this.destroy();
    ...
    for (const backend of tryList) {
      if (backend === "webgpu") {
        ...
        if (ok) return { available: true, backend: this.backend, reason: "" };
      } else {
  ...
  uploadKMap(floatBuffer: Float32Array, width: number, height: number, kRange: KRange): boolean {
    ...
    if (this.backend === "webgl") return this.uploadKMapWebGl(floatBuffer, width, height);
    if (this.backend === "webgpu") return this.uploadKMapWebGpu(floatBuffer, width, height);
    return false;
  }

  render(params: AppParams): number {
    if (!this.ready || !this.kReady) return 0;
  ...
  private async initWebGpu(): Promise<boolean> {
    ...
    try {
      ...
      if (!gpuBufferUsage) return false;
  ...
  private uploadKMapWebGl(floatBuffer: Float32Array, width: number, height: number): boolean {
    const gl = this.gl;
    if (!gl || !this.glTexture || !this.glProgram) return false;
  ...
  private uploadKMapWebGpu(floatBuffer: Float32Array, width: number, height: number): boolean {
    if (!this.device || !this.wgpuPipeline || !this.wgpuUniformBuffer) return false;

    const gpuTextureUsage = (globalThis as any).GPUTextureUsage;
    if (!gpuTextureUsage) return false;
  ...
  private renderWebGl(params: AppParams): void {
    const gl = this.gl;
    if (!gl || !this.glProgram || !this.glUniforms || !this.glTexture || !this.kReady) return;
...
function pickTryList(preference: BackendPreference): BackendKind[] {
