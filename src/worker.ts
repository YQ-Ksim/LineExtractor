import type { AppParams, ProcessOptions, ToWorkerMessage } from "./types.js";

type InitMessage = Extract<ToWorkerMessage, { type: "initImage" }>;
...
function initImage(message: InitMessage) {
...
function processFrame(id: number, rawParams: Partial<AppParams>, options: ProcessOptions) {
...
function sanitizeParams(params: Partial<AppParams>): AppParams {
  const safe: AppParams = {
...
function rebuildRegionStructureCache(tone, params, structureKey) {
  ...
  if (params.useWatershedInit && allowWatershed) {
    ...
    if (seedCount > 0) {
      ...
      const merged = felzenszwalbMergeFromWatershed(
        seg.labels,
        seedCount,
        width,
        height,
