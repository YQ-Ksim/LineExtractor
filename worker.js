...
function initImage(message) {
    ...
    state.regionCache.structureKey = "";
    state.regionCache.regionCount = 0;
    state.regionCache.labels = null;
    state.regionCache.barrier = null;
    state.regionCache.meanGray = null;
    state.regionCache.voteScore = null;
    state.regionCache.stats.felzMs = 0;
    state.regionCache.stats.felzInitialRegions = 0;
    state.regionCache.stats.felzFinalRegions = 0;
    state.regionCache.stats.felzEdgeCount = 0;
    state.regionCache.stats.watershedMs = 0;
    state.regionCache.stats.mergeMs = 0;
    state.regionCache.stats.fusionMs = 0;
...
function processFrame(id, rawParams, options) {
    ...
    if (needRgba) {
        ...
        const toneKey = buildToneKey(params, state.kVersion);
        layer3Ms = performance.now() - s;
        if (params.regionBinarize) {
            const r = performance.now();
            const regionResult = lineGuidedRegionBinarize(tone, params, toneKey);
    ...
    if (kMapBuffer)
        transfer.push(kMapBuffer);
    if (debugOutput)
        transfer.push(debugOutput.buffer);
    self.postMessage({
        type: "result",
        id,
        width: state.width,
...
function buildToneKey(params, kVersion) {
    return [
        numberKey(kVersion),
        params.normalize ? "1" : "0",
        numberKey(params.contrast),
        numberKey(params.threshold),
        numberKey(params.clipMin),
        numberKey(params.clipMax),
        numberKey(params.blur),
        numberKey(params.sharpen),
    ].join("|");
}
...
function lineGuidedRegionBinarize(tone, params, toneKey) {
    ...
    const structureKey = buildRegionStructureKey(toneKey, params);
    let cacheHit = false;
    if (state.regionCache.structureKey !== structureKey) {
        rebuildRegionStructureCache(tone, params, structureKey);
    }
    else {
        cacheHit = true;
    }
    const cache = state.regionCache;
    const labels = cache.labels;
    const barrier = cache.barrier;
    const meanGray = cache.meanGray;
    const voteScore = cache.voteScore;
    if (!labels || !barrier || !meanGray || !voteScore || cache.regionCount <= 0) {
        return {
            image: grayscaleToRgba(tone),
            felzMs: cache.stats.felzMs,
            felzInitialRegions: cache.stats.felzInitialRegions,
            felzFinalRegions: 0,
            felzEdgeCount: cache.stats.felzEdgeCount,
            watershedMs: cache.stats.watershedMs,
            mergeMs: cache.stats.mergeMs,
    ...
    const rankScore = combineVoteAndGrayRank(voteScore, meanGray, cache.regionCount, params.rankVoteWeight, params.rankGrayWeight);
    const blackRegion = chooseTopRegions(rankScore, cache.regionCount, params.regionTopPercent);
    for (let i = 0; i < n; i += 1) {
        if (barrier[i]) {
            seg.crfA[i] = 0;
            continue;
    ...
    if (params.crfEnabled && params.crfIters > 0) {
        ...
        crfRefineBinaryMask(seg.crfA, seg.crfB, seg.crfPrior, origR, origG, origB, barrier, width, height, params);
        crfMs = performance.now() - crfStart;
    }
    const out = new Uint8ClampedArray(n * 4);
    for (let i = 0; i < n; i += 1) {
        let c = 255;
        if (!barrier[i] && seg.crfA[i] >= 0.5) {
            c = 0;
        }
        const base = i * 4;
        out[base] = c;
    ...
    if (params.embedSketchLines || params.debugEmbedMask) {
        debugImage = applyAdaptiveLineTextureFromTone(out, tone, seg.crfA, width, height, params.segLineThreshold, params.segLineMinArea, params.embedSketchLines, params.debugEmbedMask);
    }
    return {
        image: out,
        felzMs: cache.stats.felzMs,
        felzInitialRegions: cache.stats.felzInitialRegions,
        felzFinalRegions: cache.regionCount,
        felzEdgeCount: cache.stats.felzEdgeCount,
        watershedMs: cache.stats.watershedMs,
        mergeMs: cache.stats.mergeMs,
        fusionMs: cache.stats.fusionMs,
...
function applyAdaptiveLineTextureFromTone(out, tone, binaryMask, width, height, lineThreshold, minLineAreaParam, enableEmbed, debugMode) {
...
function buildRegionStructureKey(toneKey, params) {
    return [
        toneKey,
        numberKey(params.segLineThreshold),
        numberKey(params.segLineDenoiseRadius),
        numberKey(params.segLineMinArea),
        numberKey(params.segDilateRadius),
        params.useWatershedInit ? "1" : "0",
        numberKey(params.wsSeedSpacing),
        numberKey(params.wsSeedMinDist),
        numberKey(params.felzK),
        numberKey(params.felzMinSize),
        numberKey(params.linePenalty),
        params.multiScaleEnabled ? "1" : "0",
        numberKey(params.multiScaleKFactor),
        numberKey(params.multiScaleMinFactor),
        params.regionMergeEnabled ? "1" : "0",
        numberKey(params.regionMergeStrength),
        numberKey(params.regionMergePasses),
    ].join("|");
}
function rebuildRegionStructureCache(tone, params, structureKey) {
    const width = state.width;
    const height = state.height;
    const n = state.totalPix;
    ...
    // White lines are treated as hard boundaries for region growth/merge.
    ...
    if (params.useWatershedInit && allowWatershed) {
        ...
        if (seedCount > 0) {
            ...
            segResult = { edgeCount: 0, initialRegions: 0, regionCount: 0 };
        }
    }
    else {
        segResult = felzenszwalbSegment(width, height, origR, origG, origB, seg.barrier, seg, params);
    ...
    if (segResult.regionCount > 1 && params.multiScaleEnabled) {
        ...
        const fusedCount = fuseFineWithCoarse(seg.labelsAux, segResult.regionCount, seg.labels, seg.barrier, width, height, seg, params);
    ...
    if (segResult.regionCount <= 0) {
        state.regionCache.structureKey = structureKey;
        state.regionCache.regionCount = 0;
        state.regionCache.labels = null;
        state.regionCache.barrier = null;
        state.regionCache.meanGray = null;
        state.regionCache.voteScore = null;
        state.regionCache.stats.felzMs = felzMs;
        state.regionCache.stats.felzInitialRegions = segResult.initialRegions;
        state.regionCache.stats.felzFinalRegions = 0;
        state.regionCache.stats.felzEdgeCount = segResult.edgeCount;
        state.regionCache.stats.watershedMs = watershedMs;
        state.regionCache.stats.mergeMs = mergeMs;
        state.regionCache.stats.fusionMs = fusionMs;
        return;
    }
    let stats = computeRegionStatsAndCompact(seg.labels, gray, seg.barrier, n, segResult.regionCount);
    if (stats.regionCount > 1 && params.regionMergeEnabled) {
        const merged = mergeSimilarAdjacentRegions(seg.labels, seg.barrier, width, height, stats.regionCount, gray, origR, origG, origB, params, seg);
    ...
    const voteScore = computeRegionScore(adjacency, stats.meanGray, stats.regionCount);
    state.regionCache.structureKey = structureKey;
    state.regionCache.regionCount = stats.regionCount;
    state.regionCache.labels = new Int32Array(seg.labels);
    state.regionCache.barrier = new Uint8Array(seg.barrier);
    state.regionCache.meanGray = stats.meanGray;
    state.regionCache.voteScore = voteScore;
    state.regionCache.stats.felzMs = felzMs;
    state.regionCache.stats.felzInitialRegions = segResult.initialRegions;
    state.regionCache.stats.felzFinalRegions = stats.regionCount;
    state.regionCache.stats.felzEdgeCount = segResult.edgeCount;
    state.regionCache.stats.watershedMs = watershedMs;
    state.regionCache.stats.mergeMs = mergeMs;
    state.regionCache.stats.fusionMs = fusionMs;
...
function computeChamferDistance(barrier, dist, width, height, n) {
    ...
    for (let y = 0; y < height; y += 1) {
        ...
        for (let x = 0; x < width; x += 1) {
            ...
            let best = dist[idx];
            if (x > 0)
                best = Math.min(best, dist[idx - 1] + d1);
            if (y > 0)
                best = Math.min(best, dist[idx - width] + d1);
            if (x > 0 && y > 0)
                best = Math.min(best, dist[idx - width - 1] + d2);
            if (x + 1 < width && y > 0)
                best = Math.min(best, dist[idx - width + 1] + d2);
            dist[idx] = best;
        }
...
function ensureSeedPerComponent(dist, barrier, labels, visited, queue, width, height, n, seedCount) {
    ...
    for (let start = 0; start < n; start += 1) {
        if (barrier[start] || visited[start])
            continue;
        let head = 0;
        let tail = 0;
        queue[tail++] = start;
...
function felzenszwalbMergeFromWatershed(labels, maxLabel, width, height, origR, origG, origB, barrier, params) {
    ...
    const edges = buildRegionMergeEdges(labels, width, height, regionCount, stats.meanR, stats.meanG, stats.meanB, barrier, params.linePenalty);
    if (edges.length === 0) {
        return { regionCount, edgeCount: 0 };
    }
...
function fuseFineWithCoarse(fineLabels, fineRegionCount, coarseLabels, barrier, width, height, seg, params) {
    if (fineRegionCount <= 1)
        return fineRegionCount;
...
function felzenszwalbSegment(width, height, origR, origG, origB, barrier, seg, params) {
    const n = width * height;
    const edgeCount = buildFelzenszwalbEdges(width, height, origR, origG, origB, barrier, seg.edgeU, seg.edgeV, seg.edgeW, seg.edgeCross, params.linePenalty);
    const parent = seg.parent;
    ...
    for (let y = 0; y < height; y += 1) {
        ...
        for (let x = 0; x < width; x += 1) {
            ...
            if (x + 1 < width) {
                ...
                if (b > 0 && b !== a)
                    addBoundaryEdge(adjacency, a, b, 1);
            }
            if (y + 1 < height) {
...
function mergeSimilarAdjacentRegions(labels, barrier, width, height, regionCount, gray, origR, origG, origB, params, seg) {
    ...
    for (let pass = 0; pass < passes; pass += 1) {
        ...
        for (let i = 0; i < edges.length; i += 1) {
            ...
            const closeAndSmall = edge.len >= minBoundary && minArea <= smallAreaThr && colorDiff <= colorThr && grayDiff <= grayThr;
            const longBoundaryClose = edge.len >= minBoundary * 3 && colorDiff <= colorThr * 0.75 && grayDiff <= grayThr * 0.8;
            if (!(veryClose || closeAndSmall || longBoundaryClose))
                continue;
            if (size[ra] < size[rb]) {
                const t = ra;
                ra = rb;
