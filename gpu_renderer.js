...
export class HybridGpuRenderer {
  ...
  uploadKMap(floatBuffer, width, height, kRange) {
    if (!this.ready || !(floatBuffer instanceof Float32Array)) {
      return false;
    }
  ...
  renderWebGl(params) {
    const gl = this.gl;
    if (!gl || !this.glProgram || !this.glUniforms) return;
  ...
  renderWebGpu(params) {
    ...
    uniforms[0] = this.kRange.min;
    uniforms[1] = this.kRange.max;
    uniforms[2] = params.normalize ? 1 : 0;
    uniforms[3] = clampNumber(params.contrast, 0, 10, 1);
    uniforms[4] = clampNumber(params.threshold, 0, 1, 0);
    uniforms[5] = clipMin;
    uniforms[6] = clipMax;
    uniforms[7] = blur;
    uniforms[8] = sharpen;
    uniforms[9] = this.width;
    uniforms[10] = this.height;

    this.device.queue.writeBuffer(this.wgpuUniformBuffer, 0, uniforms.buffer);
